...
int SearchFirstFit(BST* tree, int root, int size)
{
    if (root == -1) return -1;

    // search left first (lower addresses)
    int left = SearchFirstFit(tree, tree->bst[root].left, size);
    ...
    // search right
    return SearchFirstFit(tree, tree->bst[root].right, size);
}
...
int main(void)
{
    ...
    {
        printf("\n========== "ANSI_COLOR_GREEN"MEMORY MANAGEMENT MENU ("ANSI_COLOR_CYAN"First Fit"ANSI_COLOR_RESET")==========\n");
